...
const fs = require("fs");          // 🔹 1. Ajoutez fs
const sqlite3 = require("sqlite3").verbose();  // 🔹 2. Ajoutez sqlite3
const { app } = require("electron");
const { getDb, dbAll, dbRun } = require(path.join(__dirname, "..", "db.js"));

// 🔹 3. Supprimez la ligne `let db = null;` (inutile car on utilise getDb())
...
async function getPhotoSettings(filePath) {
    try {
        const rows = await allAsync(`
            SELECT picture_control_json, transform_json
            FROM catalog_photos
            WHERE file_path = ?
        `, [filePath]);

        if (rows.length === 0) {
            console.log("⚠️ Aucun réglage trouvé pour:", filePath);
            return null;
        }

        const row = rows[0];
        const pcData = row.picture_control_json ? JSON.parse(row.picture_control_json) : {};
        const transformData = row.transform_json ? JSON.parse(row.transform_json) : {};

        return { ...pcData, transform: transformData };
    } catch (err) {
