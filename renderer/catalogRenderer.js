...
async function refreshGrid() {
    ...
    // Génération des cartes avec redirection vers Studio
    photos.forEach(photo => {
        ...
        // CLIC SUR UNE PHOTO : Utilise la fonction globale créée dans app.js
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Attacher l'événement d'importation aux deux ID possibles de bouton
    ...
    // Gestion du basculement de vues
    const btnOpenCatalog = document.getElementById('btnOpenCatalog');
    const btnReturnFromCatalog = document.getElementById('btnReturnToStudioFromCatalog') || document.getElementById('btnReturnToStudio');

    if (btnOpenCatalog) {
        btnOpenCatalog.addEventListener('click', () => {
            if (typeof window.switchToView === "function") {
                window.switchToView('view-catalog');
            }
            refreshFolders();
            refreshGrid();
        });
    ...
    if (btnReturnFromCatalog) {
        btnReturnFromCatalog.addEventListener('click', () => {
            if (typeof window.switchToView === "function") {
